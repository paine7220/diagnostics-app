...
const DiagEngine = (() => {
  ...
  function misfireGuide(title) {
    return {
      ...
      diySteps: [
        'Park on level ground, set the brake, engine off, key out. If the check-engine light was flashing while you drove, do not keep driving hard — that can melt the catalytic converter. Let a hot engine sit before you pull plugs.',
        'Write the exact misfire code (P0300 random vs P0301 cylinder 1, and so on). Cylinder numbers are not always “front of the engine = 1.” Use the firing-order sticker, intake stamp, or a year/make/engine diagram.',
        'Open the hood. Pull the oil dipstick, wipe it, reinsert, and read it. If oil is very low, add the correct oil and stop until you know why it was low. Low oil and a misfire together can be a rod or a VVT problem, not just a plug.',
        'Look at the battery posts. If they are white/green and crusty, disconnect negative first, clean them, and tighten. A weak dirty battery can cause random misfires and false sensor codes. Recheck after cleaning before you buy coils.',
        'Find the ignition coils (usually a plastic pack on top of each spark plug on coil-on-plug engines). Unplug one coil on a misfiring cylinder, swap it with a different cylinder, then drive a short loop or recheck codes. If the misfire moves to the other cylinder, replace that coil. If it stays, the coil is not the cause.',
        'If the misfire stays on the same cylinder, pull that spark plug with a spark-plug socket after blowing dirt out of the well. Look for oil (valve-cover gasket), coolant (head gasket), a burned electrode, or a huge gap. Replace old plugs as a set with the exact part, gapped only if the maker says to, to the door-jamb spec.',
        'On engines with plug wires, twist the boot, do not yank the wire. Route new wires in the factory looms off the exhaust. A wire on the wrong tower is a misfire you just created.',
        'With the engine off, squeeze and inspect the big rubber intake tube after the air box and the PCV hose. Cracks and loose clamps let in extra air and make a misfire or a lean code. Replace split boots; tape is not a repair.',
        ...
        'Compression or leak-down on a dead cylinder that will not follow a coil or plug: hiss in the intake is a valve, in the crankcase is rings, bubbles in the radiator is a head gasket. That is the engine-rebuild path, not another coil.',
        'Stop and go to a shop if you hear knock, see white or blue smoke, find milky oil, or you do not have the tools. One change at a time, then recheck the code. Clear codes only after the repair so you can see if it returns.'
      ]
    };
  }
  ...
  const COMMON_JOBS = (typeof window !== 'undefined' && Array.isArray(window.COMMON_JOBS_DATA) && window.COMMON_JOBS_DATA.length)
    ? window.COMMON_JOBS_DATA
    : ((typeof globalThis !== 'undefined' && Array.isArray(globalThis.COMMON_JOBS_DATA) && globalThis.COMMON_JOBS_DATA.length)
      ? globalThis.COMMON_JOBS_DATA
      : []);
  ...
  function listRepairPlaybooks() {
    ...
    COMMON_JOBS.forEach((job) => {
      const extra = [];
      if (job.time) extra.push('Typical time: ' + job.time);
      if (job.tools && job.tools.length) extra.push('Tools: ' + job.tools.join('; '));
      if (job.parts && job.parts.length) extra.push('Parts: ' + job.parts.join('; '));
      (job.warnings || []).forEach((w) => extra.push(w));
      add('How-to — ' + job.title, extra.concat(job.steps || []));
    });
